function varargout = ScadaGUIH(varargin)
% SCADAGUIH M-file for ScadaGUIH.fig
%      SCADAGUIH, by itself, creates a new SCADAGUIH or raises the existing
%      singleton*.
%
%      H = SCADAGUIH returns the handle to a new SCADAGUIH or the handle to
%      the existing singleton*.
%
%      SCADAGUIH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SCADAGUIH.M with the given input arguments.
%
%      SCADAGUIH('Property','Value',...) creates a new SCADAGUIH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ScadaGUIH_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ScadaGUIH_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ScadaGUIH

% Last Modified by GUIDE v2.5 25-Oct-2011 00:01:10

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ScadaGUIH_OpeningFcn, ...
                   'gui_OutputFcn',  @ScadaGUIH_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ScadaGUIH is made visible.
function ScadaGUIH_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ScadaGUIH (see VARARGIN)

% Choose default command line output for ScadaGUIH
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ScadaGUIH wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ScadaGUIH_OutputFcn(hObject, eventdata, handles) 
